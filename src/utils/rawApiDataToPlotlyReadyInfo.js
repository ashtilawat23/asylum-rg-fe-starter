const rawApiDataToPlotlyReadyInfo = (view, office, data) => {
  ...
  for (let yearResults of data[0]['fiscalSummary']['yearResults']) {
  ...
  for (let office of data[0]['fiscalSummary']['yearResults']) {
  ...
  if (!office || office === 'all') {
    switch (view) {
      case 'time-series':
        ...
        for (let officeName of data[0]['fiscalSummary']['yearResults']) {
      ...
      case 'citizenship':
        rowsForTable = [];
        for (let item of data[1]['citizenshipSummary']) {
          rowItem = {
        ...
        for (let country of data[1]['citizenshipSummary']) {
    ...
    switch (view) {
      case 'time-series':
        rowsForTable = [];
        data[0].fiscalSummary.yearResults.sort(
          (a, b) => a.fiscal_year - b.fiscal_year
        );
        for (let i = 0; i < data[0].fiscalSummary.yearResults.length; i++) {
          if (
            data[0].fiscalSummary.yearResults[i].yearData.filter(
              dataItem => dataItem.office === office
            )[0]
          ) {
            const officeObj = data[0].fiscalSummary.yearResults[
              i
            ].yearData.filter(dataItem => dataItem.office === office)[0];
            rowItem = {
              'Fiscal Year': data[0].fiscalSummary.yearResults[i].fiscal_year,
